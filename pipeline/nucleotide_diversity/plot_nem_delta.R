...
for (col in c("nem","prev_nem","delta_nem","delta_ort")) {
  iklim[[col]] <- suppressWarnings(as.numeric(iklim[[col]]))
}

# Mutlak delta nem ve sıcaklık
...
climate_vars <- list(
  ...
  list(col="delta_nem",    lab="Delta Humidity (current - prev, %)"),
  list(col="abs_delta_nem",lab="|Delta| Humidity (change magnitude, %)"),
  list(col="delta_ort",    lab="Delta Mean Temp (current - prev, °C)"),
  list(col="abs_delta_ort",lab="|Delta| Mean Temp (change magnitude, °C)")
)
...
# 5. PDF
# ============================================================
pdf(file.path(DIR_1k, "humidity_delta_vs_diversity.pdf"), width=18, height=12)
plots <- list()
...
dev.off()
cat("Saved: humidity_delta_vs_diversity.pdf\n")
...
cat("\n=== Korelasyon Tablosu ===\n")
print(cor_table)
fwrite(cor_table, file.path(DIR_1k, "humidity_delta_correlations.tsv"), sep="\t")
cat("\nDone!\n")
