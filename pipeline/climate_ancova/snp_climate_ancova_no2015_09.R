#=============================================================================
# SNP Allele Frequency - Climate ANCOVA Analysis (2015_09 EXCLUDED, 2017 climate fixed)
# Models: Season-only, Year-only, Season*Year, Season+Year+Climate(PCA)
# Input: mega_all_years_freq_diff_rc
# Output: CLIMATE_ANALYSIS/no2015_09/
#=============================================================================
library(data.table)
library(foreach)
library(doParallel)
...
OUT_DIR <- "/arf/scratch/ssenkal/selin/sekanslar/all_years_26/CLIMATE_ANALYSIS/no2015_09"
...
# ---- 2015_09 (maa_3) haric tut - dusuk coverage nedeniyle (%58.8 valid SNP) ----
sample_meta <- sample_meta[sample_id != "maa_3"]
cat("Kalan kohort sayisi:", nrow(sample_meta), "\n")
...
#-----------------------------------------------------------------------------
# 2. CLIMATE DATA
# NOT: 2017 Agustos/Eylul icin genetik ornek yok ama gercek iklim istasyonu
# verisi VAR (Yesiloz_Iklim_Verileri.xlsx'ten dogrulandi) - eklendi ki delta
# hesaplari 2018'i yanlislikla 2016'dan degil, dogru sekilde 2017'den hesaplasin.
#-----------------------------------------------------------------------------
...
avg_temp <- make_clim("avg_temp", list(
  ...
  "2017"=c("6"=19.7,"8"=24.2,"9"=21.9,"10"=11.8),
...
min_temp <- make_clim("min_temp", list(
  ...
  "2017"=c("6"=12.7,"8"=16.3,"9"=13.0,"10"=5.7),
...
max_temp <- make_clim("max_temp", list(
  ...
  "2017"=c("6"=27.8,"8"=32.8,"9"=31.6,"10"=19.9),
...
precip <- make_clim("precipitation", list(
  ...
  "2017"=c("6"=91.2, "8"=52.6,"9"=3.0, "10"=33.4),
...
avg_humidity <- make_clim("avg_humidity", list(
  ...
  "2017"=c("6"=63.2,"8"=51.1,"9"=38.9,"10"=61.7),
...
min_humidity <- make_clim("min_humidity", list(
  ...
  "2017"=c("6"=32.5,"8"=22.3,"9"=16.8,"10"=31.7),
...
max_humidity <- make_clim("max_humidity", list(
  ...
  "2017"=c("6"=89.0,"8"=80.5,"9"=64.0,"10"=85.3),
...
#-----------------------------------------------------------------------------
# 3. PCA ON CLIMATE VARIABLES (across the 25 samples)
#-----------------------------------------------------------------------------
...
res <- foreach(i = 1:nrow(snps), .combine = rbind, .packages = c("data.table","car"), .errorhandling = "remove") %dopar% {
...
# 7. SAVE FULL RESULTS
#-----------------------------------------------------------------------------
full_out <- file.path(OUT_DIR, "snp_climate_ancova_full_no2015_09.tsv")
...
sig_tests <- list(
  ...
  ancova_season = res[q3_season < 0.05, .(chr, pos, p3_season, q3_season, p3_year, q3_year, p3_interaction, q3_interaction)],
  ancova_year = res[q3_year < 0.05, .(chr, pos, p3_season, q3_season, p3_year, q3_year, p3_interaction, q3_interaction)],
  ancova_interact = res[q3_interaction < 0.05, .(chr, pos, p3_season, q3_season, p3_year, q3_year, p3_interaction, q3_interaction)],
  climate_PC1 = res[q4_PC1 < 0.05, .(chr, pos, p4_season, q4_season, p4_year, q4_year, p4_PC1, q4_PC1, p4_PC2, q4_PC2)],
  climate_PC2 = res[q4_PC2 < 0.05, .(chr, pos, p4_season, q4_season, p4_year, q4_year, p4_PC1, q4_PC1, p4_PC2, q4_PC2)]
)

for (nm in names(sig_tests)) {
  out_path <- file.path(OUT_DIR, paste0("sig_", nm, "_no2015_09.tsv"))
...
#-----------------------------------------------------------------------------
loadings_out <- file.path(OUT_DIR, "climate_pca_loadings_no2015_09.tsv")
